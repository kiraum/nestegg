 ...
 */
function getFriendlyName(typeId) {
    const nameMap = {
        'cdb': 'CDB (Prefixado)',
        'lci': 'LCI (Prefixado)',
        'lca': 'LCA (Prefixado)',
 ...
 */
function getInvestmentCategory(typeId) {
 ...
 */
function getRateDescription(typeId) {
    const rateDescMap = {
 ...
 */
export function renderInvestmentTypes(types, container, onToggle, selectedTypes = new Set(), savedValues = {}) {
    container.innerHTML = '';
    ...
    categoryOrder.forEach(category => {
        ...
        categories[category].forEach(type => {
            ...
            item.addEventListener('click', () => {
                ...
                if (needsParams) {
                    const paramSection = item.nextElementSibling;
 ...
 */
export function renderSelectedInvestments(selectedTypes, allTypes, container, savedValues = {}) {
 ...
 */
export function renderInvestmentResult(result, container) {
    container.innerHTML = '';
 ...
 */
export function renderComparisonResults(results, container) {
    container.innerHTML = '';
    if (results.length === 0) {
